...
const createEntityController = async (req: Request, res: Response) => {
    try {
        ...
        const checkExistingEntity = await prisma.siswa.findUnique({where: {absen}})

        if (checkExistingEntity != null) return res.status(401).json({messsage: "Entity is exist", checkExistingEntity})
