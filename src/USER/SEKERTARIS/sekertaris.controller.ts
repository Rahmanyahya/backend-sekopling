...
const addEventController = async (req: Request, res: Response) => {
  try {
    ...
    const newData: addNewEvent = {
      pelaksanaan: new Date(pelaksanaan),
      judul,
      deskripsi
    }
...
const updateEventController = async (req: Request, res: Response) => {
  try {
    ...
    const newData: addNewEvent = {
      pelaksanaan: new Date(pelaksanaan) ?? idValidation.pelaksanaan,
      judul: judul ?? idValidation.judul,
      deskripsi: deskripsi ?? idValidation.deskripsi
    }
...
const addJadwalController = async (req: Request, res: Response) => {
  try {
    ...
    const checkExistingJadwal = await prisma.jadwal.findFirst({
      where: {
        AND: [
          { hari },
          { start },
...
const updateJadwalController = async (req: Request, res: Response) => {
  try {
    ...
    const newData: addNewJadwal = {
      hari = hari ?? idValidation.hari,
      guruID = guruID ?? idValidation.guruID,
      start = new Date(start) ?? idValidation.start,
      end = new Date(end) ?? idValidation.end
    }
